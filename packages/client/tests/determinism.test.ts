...
describe('determinism harness (client, jsdom)', () => {
  it('runs in the resolved zone, so a zone-less date literal is the instant it reads as', () => {
    ...
    // in Denver. Checked against RUN_TZ's hand-checked facts rather than against
    // UTC's literals, because `test:dst` runs this same file in America/New_York
    // — where the two rows differ by an hour, which is the property that makes
    // that leg worth running. Both halves of the year, so a zone that is UTC only
    // in winter (Europe/London) cannot satisfy it.
  ...
  it('resolves Intl to the resolved zone inside jsdom', () => {
    // jsdom carries its own Intl, so this is a genuinely separate claim from the
    // node tiers': a pin applied to `process.env.TZ` that ICU inside jsdom never
    // saw would leave every rendered date reading in the machine's zone.
  ...
  it('keeps shuffle and the pins in the runner config', () => {
    ...
    // RUN_TZ, not PINNED_TZ: the config reads RUN_TZ so the DST leg is not
    // silently clobbered back to UTC, and a hardcoded constant would only ever
    // be asserted in the leg that does not need it.
