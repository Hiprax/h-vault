 ...
 *     this suite ten times; pointed at `junit-client.xml` it would overwrite the
 ...
 *     `--outputFile.junit=…`, measured, so the run still wrote `junit-client.xml`.
 *     Hence a config file.
 *
 ...
 *     `test:unit` enforces the thresholds on every push and keeps doing so.
 *
...
const junitReporter: ['junit', { outputFile: string }] = [
  'junit',
  {
    outputFile: path.resolve(
      ...
      '../../.testfortress/reports/junit-flake-client.xml',
    ),
  },
];
