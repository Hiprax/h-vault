...
| `HVAULT_EDGE_SUBNET`, `HVAULT_DATA_SUBNET`                  | Only if `172.28.x` is already taken here. An overlap is fatal: Docker refuses to create the network.                                 |
...
| `docker compose up` fails: "Pool overlaps with other one on this address space" | Another Docker network already owns `172.28.0.0/24` or `172.28.1.0/24`. Set `HVAULT_EDGE_SUBNET` / `HVAULT_DATA_SUBNET` to free blocks, and give each stack its own `HVAULT_HTTP_PORT`.                                                                                                                                                                                                                                                                                                               |
...
| `MongoParseError` / auth failures on boot                                       | `MONGO_ROOT_PASSWORD` contains a character that is URI syntax (`@ : / ? #`). Regenerate it with `openssl rand -hex 32`.                                                                                                                                                                                                                                                                                                                                                                               |
...
| `HVAULT_VERSION`      | `0.1.1`          | Image tag for the three first-party images. Keep it equal to `package.json` |
| `HVAULT_EDGE_SUBNET`  | `172.28.0.0/24`  | Nginx ↔ app, plus the app's egress                                          |
| `HVAULT_DATA_SUBNET`  | `172.28.1.0/24`  | App ↔ MongoDB. Internal: no published port, no route out                    |
| `TRUST_PROXY_HOPS`    | `2`              | Becomes the app's `TRUST_PROXY`. Must match reality exactly                 |
| `MONGO_ROOT_USERNAME` | `hvault`         | Created on the database's first boot only                                   |
...
| Health          | 60 / IP     | 1 min  | health and public config                                                                                                                                 |
| Metrics         | 60 / IP     | 1 min  | the metrics endpoint                                                                                                                                     |
